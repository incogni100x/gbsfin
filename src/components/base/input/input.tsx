...
export interface InputBaseProps extends Omit<AriaInputProps, "size" | "className"> {
  ...
  /** Custom interactive element rendered in the trailing slot. */
...
const inputStyles = sortCx({
  field: [
    ...
    "rounded-2lg",
    "border border-border-button-default bg-transparent text-foreground-icon-tertiary",
...
export function Input({
  ...
  const resolvedType =
    isPasswordField && isPasswordVisible ? "text" : textFieldProps.type;

  const passwordVisibilityToggle = isPasswordField ? (
    <button
      aria-label={isPasswordVisible ? "Hide password" : "Show password"}
      aria-pressed={isPasswordVisible}
      className="grid size-7 shrink-0 cursor-pointer place-items-center rounded-md text-foreground-icon-secondary outline-none transition-colors hover:text-accent-600 focus-visible:ring-2 focus-visible:ring-border-focus-ring"
      ...
      {({ isRequired, isInvalid }) => (
        <>
          ...
          <InputBase
            ...
            trailingAddon={passwordVisibilityToggle ?? trailingAddon}
