...
export async function getLoanOptions() {
  ...
  const [types, planEligibility, purposes, partners] = await Promise.all([
    client
      .from("loan_types")
      .select("id, name, description, is_investment")
      .eq("is_active", true)
      .order("id"),
    client
      .from("loan_type_plans")
      .select(
        "loan_type_id, loan_plans!inner(id, duration_months, annual_interest_rate, is_active)",
      ),
    client
      .from("loan_purposes")
      .select("label")
      ...
      .order("display_order"),
    client
      .from("investment_loan_partners")
      .select("name")
      .eq("is_active", true)
      .order("display_order"),
  ]);
  [types, planEligibility, purposes, partners].forEach(({ error }) =>
    check(error),
  );
  return {
    types: types.data,
...
export async function requestLoan({
  ...
  const { data, error } = await requireSupabase().rpc("request_loan", {
    p_account_id: accountId,
    p_amount: amount,
    p_loan_type_id: loanTypeId,
    p_plan_id: planId,
    p_reason: reason,
  });
  check(error);
