...
export default function TransferPage() {
  ...
  return <section>
    ...
    <section aria-labelledby="bank-transfers-heading">
      ...
      <p className="text-body-2-medium mt-1 mb-4 text-[var(--color-text-secondary)]">Transfers between your own non-escrow accounts complete immediately. Linked-bank and escrow transfers require approval.</p>
      {accountsPending ? <ContentSkeleton label="Loading bank accounts" /> : accounts.length === 0 ? <p className="text-body-medium text-[var(--color-text-secondary)]">No bank accounts are available.</p> : <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {accounts.map((account) => <LayerCard className="w-full !bg-[var(--color-background-primary-default)] text-[var(--color-text-primary)] ring-[var(--color-separator-border)]" key={account.id}>
          ...
          <LayerCard.Primary className="!bg-[var(--color-accent-600)] px-4 py-4 text-[var(--color-neutral-50)] ring-[var(--color-separator-border)]"><strong className="financial-number text-title-2-medium">{formatCurrency(account.balance, account.currency)}</strong><span className="text-body-2-medium text-[var(--color-neutral-50)]">Available balance</span><div className="mt-3"><BankTransferDialog account={account} accounts={accounts} linkedAccounts={linkedAccounts} /></div></LayerCard.Primary>
        </LayerCard>)}
      </div>}
    </section>
